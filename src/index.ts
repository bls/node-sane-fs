...
export type DataT = string | Buffer;
...
export let readAsync: (fd: number, buffer: Buffer, offset: number, length: number, position: number) => Promise<[number, Buffer]> = promisify(fs.read);
